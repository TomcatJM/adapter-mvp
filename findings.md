...
- 提交说明中放 `YUNXIAO_TASK_DISPLAY_ID=<页面展示ID>` 可以稳定绑定 workflow，真实绑定来源仍为 `commit_message_yunxiao_task_id`；`云效id`、`云效任务` 等中文 key、旧 `YUNXIAO_TASK_ID` 和内部 `workitemIdentifier` 也兼容。
- 标准提交标题推荐：`feat: <说明> YUNXIAO_TASK_DISPLAY_ID=VEGZ-1186`；如果标题放不下，可在正文单独一行写 `云效展示ID：VEGZ-1186` 或 `云效任务：VEGZ-1186`。
