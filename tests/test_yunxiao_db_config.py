...
class YunxiaoDbConfigTest(unittest.TestCase):
    ...
    def test_find_yunxiao_project_member_returns_none_without_normalized_member(self) -> None:
        cursor = MagicMock()
        cursor.__enter__.return_value = cursor
        cursor.__exit__.return_value = False
        ...
        sql = cursor.execute.call_args.args[0]
        self.assertIn("adapter_yunxiao_project_member_relation", sql)
        self.assertNotRegex(sql, r"FROM\s+adapter_yunxiao_project_member\s")
