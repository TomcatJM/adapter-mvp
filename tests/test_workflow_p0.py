...
class WorkflowP0Test(unittest.TestCase):
    ...
    def test_submit_requirement_ignores_empty_task_heading_when_backfilling_owner(self) -> None:
        from app.models import WorkflowRequirementRequest
        from app.workflow import submit_requirement

        workflow = {
            "workflowId": "wf-test-empty-heading",
            "status": "DOC_READ",
            "context": {
                "dingtalk": {
                    "read": {
                        "document": {
                            "result": {
                                "data": [
                                    ...
                                    {
                                        "blockType": "heading",
                                        "heading": {"text": "线索增加字段是否为测试", "level": "heading-5"},
                                    },
                                    {"blockType": "paragraph", "paragraph": {"text": "负责人：姬志猛"}},
                                    {"blockType": "heading", "heading": {"text": None, "level": "heading-5"}},
                                    {
                                        "blockType": "heading",
                                        "heading": {"text": "线索列表需要展示当前字段", "level": "heading-5"},
                                    },
                                    {"blockType": "paragraph", "paragraph": {"text": "负责人：谢铭琪"}},
                                ]
                            }
                    ...
                    demands=[
                        {
                            ...
                            "items": [
                                {"itemIndex": 1, "title": "线索增加字段是否为测试", "parentDemandTitle": "需求一"},
                                {"itemIndex": 2, "title": "线索列表需要展示当前字段", "parentDemandTitle": "需求一"},
                            ],
                        }
                    ],
                ),
    ...
    def test_submit_requirement_backfills_task_content_lines_from_dingtalk_read(self) -> None:
        from app.models import WorkflowRequirementRequest
        from app.workflow import submit_requirement

        workflow = {
            "workflowId": "wf-test-content",
            "status": "DOC_READ",
            "context": {
                "dingtalk": {
                    "read": {
                        "document": {
                            "result": {
                                "data": [
                                    {"blockType": "paragraph", "paragraph": {"text": "项目名：02-园务系统"}},
                                    {"blockType": "heading", "heading": {"text": "需求二：", "level": "heading-4"}},
                                    {
                                        "blockType": "heading",
                                        "heading": {"text": "1. 跟进记录增加字段是否为测试", "level": "heading-5"},
                                    },
                                    ...
                                    {
                                        "blockType": "heading",
                                        "heading": {"text": "2. 列表增加展示此字段", "level": "heading-5"},
                                    },
                    ...
                    demands=[
                        {
                            ...
                            "items": [
                                {"itemIndex": 1, "title": "跟进记录增加字段是否为测试", "parentDemandTitle": "需求二"},
                                {"itemIndex": 2, "title": "列表增加展示此字段", "parentDemandTitle": "需求二"},
                            ],
                        }
                    ],
                ),
        ...
        self.assertEqual(demand["description"], "跟进记录增加字段是否为测试，列表增加展示此字段")
        self.assertEqual(demand["items"][0]["contentLines"], ["跟进记录增加字段是否为测试，字段为int，默认为0"])
        self.assertEqual(demand["items"][1]["contentLines"], ["列表增加展示此字段，字段为int，默认为0"])

    def test_submit_requirement_backfills_task_content_by_position_when_title_differs(self) -> None:
        from app.models import WorkflowRequirementRequest
        from app.workflow import submit_requirement

        workflow = {
            "workflowId": "wf-test-content-position",
            "status": "DOC_READ",
            "context": {
                "dingtalk": {
                    "read": {
            ...
            result = submit_requirement(
                ...
                WorkflowRequirementRequest(
                    ...
                    demands=[
                        {
                            "demandIndex": 1,
                            "title": "跟进记录增加测试字段并在列表展示",
                            "description": "跟进记录增加字段是否为测试，列表增加展示此字段",
                            "items": [
                                {
                                    "itemIndex": 1,
                                    "title": "跟进记录增加字段是否为测试",
                                    "parentDemandTitle": "跟进记录增加测试字段并在列表展示",
                                },
                                {
                                    "itemIndex": 2,
