...
| 7. 云效人员表、项目人员关联与默认负责人 | complete | 新增 `adapter_yunxiao_member` 和 `adapter_yunxiao_project_member_relation`，支持指定负责人和项目默认负责人；旧 `adapter_yunxiao_project_member`、`default_assignee` 保留为兼容兜底。 |
...
- 云效负责人按人员表和项目人员关联表维护，解析顺序为：请求指定 `assigneeId/assigneeName` -> 项目人员关联默认负责人 -> 旧项目人员表 -> 项目表旧 `default_assignee` 兜底；指定负责人不存在时显式失败。
...
- 标准提交说明优先把页面展示 ID 放进 `YUNXIAO_TASK_DISPLAY_ID`，例如 `YUNXIAO_TASK_DISPLAY_ID=VEGZ-1186`；同时宽松兼容 `云效id`、`云效任务` 等中文 key，旧 `YUNXIAO_TASK_ID` 仍兼容页面展示 ID 和内部 `workitemIdentifier`。
