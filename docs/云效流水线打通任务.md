...
- 不带 `WORKFLOW_ID` 时，可按 `YUNXIAO_TASK_ID` / `REQUIREMENT_ID` / `workitemId`、提交说明里的 `WORKFLOW_ID` / `YUNXIAO_TASK_ID`、`pipelineId + buildNumber`、`branchName + commitId` 兜底绑定 workflow。`YUNXIAO_TASK_ID` 优先使用页面展示 ID，例如 `VEGZ-1186`，内部 `workitemIdentifier` 也兼容。
...
5. 提交说明中的 `YUNXIAO_TASK_ID` / `云效任务ID`，优先填页面展示 ID，例如 `VEGZ-1186`
