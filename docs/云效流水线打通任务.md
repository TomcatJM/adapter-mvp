...
- 不带 `WORKFLOW_ID` 时，可按 `YUNXIAO_TASK_DISPLAY_ID` / `yunxiaoTaskDisplayId` / `YUNXIAO_TASK_ID` / `REQUIREMENT_ID` / `workitemId`、提交说明里的 `WORKFLOW_ID` / 任务展示 ID、`pipelineId + buildNumber`、`branchName + commitId` 兜底绑定 workflow。`YUNXIAO_TASK_DISPLAY_ID` 优先使用页面展示 ID，例如 `VEGZ-1186`，旧 `YUNXIAO_TASK_ID` 仍兼容页面展示 ID 和内部 `workitemIdentifier`。
...
5. 提交说明中的 `YUNXIAO_TASK_DISPLAY_ID` / `yunxiaoTaskDisplayId` / `云效展示ID`，填页面展示 ID，例如 `VEGZ-1186`；旧 `YUNXIAO_TASK_ID` / `云效任务ID` 仍兼容
