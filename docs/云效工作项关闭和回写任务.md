...
- [ ] 使用测试云效工作项完成一次回写和关闭。当前已跑到 `APIFOX_SYNCED`，但关单被云效鉴权配置阻塞。
...
- `done_status_id=100014` 已可用于完成状态；真实关单还需要把云效账号从 `legacy_token` 切到 `acs_ak`。
...
- 解决方案：补 `acs_ak` 账号配置并把 `jdb-school-crm` / `校CRM` 项目映射切到该账号，然后调用 `/workflow/{id}/resolve` 恢复到 `APIFOX_SYNCED`，再 `/workflow/{id}/advance` 重试关单。
