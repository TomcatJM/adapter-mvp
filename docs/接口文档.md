...
- If the commit message explicitly contains `WORKFLOW_ID` or `YUNXIAO_TASK_ID` but Adapter cannot find a matching workflow, it stops fallback binding and returns `workflow not found` to avoid cross-project misbinding.
