...
- 如果 `sprint_id` 为空，且结构化需求里没有 `version`，创建云效需求和任务时不写迭代字段，允许为空。
- 如果 `sprint_id` 为空，但结构化需求里有 `version`，personal token 链路只允许按版本号精确匹配云效迭代，并把匹配到的迭代 ID 写入创建 payload 的 `sprint` 字段。
- 如果文档写了版本号但云效不存在同名迭代，Adapter 会按文档版本号创建同名迭代后再创建需求和任务；禁止把 `V1.0.0` 猜到 `CRM-集团-V1.0.0` 这类相似迭代上。
...
| 负责人 | `assigneeId` / `assigneeName`，否则项目默认负责人 | 优先查 `adapter_yunxiao_member` + `adapter_yunxiao_project_member_relation`；`default_assignee` 只做兜底 |
