...
当前实现使用 DB 优先的配置模型，拆成账号、项目、人员和项目人员关联表：
...
- `adapter_yunxiao_project_config`：维护业务项目名到云效 `organization_id`、`project_id`、可选 `sprint_id`、需求工作项类型、任务工作项类型等字段的映射，保留 `default_assignee` 作为旧配置兜底。
- `adapter_yunxiao_member`：维护全局人员，字段包括姓名、云效账号 ID、是否启用。
- `adapter_yunxiao_project_member_relation`：维护项目和人员的关联关系，字段包括项目名、云效账号 ID、是否项目默认负责人、是否启用。
- `adapter_yunxiao_project_member`：旧兼容表，短期保留读写，数据会回填到新人员表和关联表。

迭代字段规则：

- 如果项目配置了 `sprint_id`，创建云效需求和任务时直接使用该迭代 ID。
- 如果 `sprint_id` 为空，但结构化需求里有 `version`，personal token 链路会按版本号查询云效迭代，并把匹配到的迭代 ID 写入创建 payload 的 `sprint` 字段。
- 如果文档写了版本号但云效找不到唯一匹配迭代，创建失败并提示补云效迭代或固定配置 `sprint_id`，不能静默创建空迭代工作项。

父子关系规则：

- 先创建云效“需求”工作项。
- 再创建该需求下的云效“任务”工作项。
- personal token 链路创建子任务时，必须把父需求内部 ID 作为 `parentId` 传给云效；Adapter context 同时保留 `parentIdentifier` 便于审计和流水线回查。
...
CREATE TABLE adapter_yunxiao_member (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '自增主键',
...
CREATE TABLE adapter_yunxiao_project_member_relation (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '自增主键',
    project_name VARCHAR(128) NOT NULL COMMENT '业务项目名称，例如 jdb-school-crm',
...
python scripts/upsert_yunxiao_config.py \
  ...
  --task-workitem-type-identifier <任务工作项类型ID> \
...
| 负责人 | `assigneeId` / `assigneeName`，否则项目默认负责人 | 优先查 `adapter_yunxiao_member` + `adapter_yunxiao_project_member_relation`；旧 `adapter_yunxiao_project_member` 和 `default_assignee` 只做兜底 |
...
- [x] 创建成功后 workflow 保存内部 `yunxiaoTaskId`，并在可获取时保存页面展示 ID `yunxiaoTaskDisplayId`。
- [x] 重复调用不会重复创建。
