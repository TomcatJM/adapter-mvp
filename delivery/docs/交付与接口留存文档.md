...
2. 按云效流水线 ID 查询数据库映射 Apifox 项目配置（当前推荐）：

云效 Webhook 入参固定为 `task` / `sources` / `globalParams`，不从 URL query/path 传项目参数时，Adapter 可按 `task.pipelineId` 映射项目：

```text
adapter_apifox_pipeline_config.pipeline_id -> apifox_project_config_id
adapter_apifox_project_config.id -> apifox_project_id
```

当前已验证：

```text
adapter_apifox_pipeline_config: 4437990 -> apifox_project_config_id=<jdb-order配置ID>
adapter_apifox_pipeline_config: 4989239 -> apifox_project_config_id=<jdb-school-gmc配置ID>
...
2. 数据库 adapter_apifox_pipeline_config：pipeline_id -> apifox_project_config_id
3. 数据库 adapter_apifox_project_config：id -> apifox_project_id
4. 环境变量 APIFOX_PROJECT_<KEY>_ID
```
...
adapter_apifox_pipeline_config.apifox_project_config_id=<jdb-school-gmc配置ID>
...
projectNameSource=database_pipeline_config_id
