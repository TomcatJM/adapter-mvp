...
- [x] `APIFOX_SYNCED` 状态下调用 `/workflow/{id}/advance` 可以关闭云效工作项。
- [x] `YUNXIAO_TASK_CLOSED` 状态下重复调用不会重复关闭。
- [x] 云效已关闭时能补偿推进 workflow。
- [x] 关闭失败不会误报成功，会进入 `NEEDS_HUMAN`。
- [x] 回写内容里不包含密钥。
...
- [x] 单测覆盖正常关闭。
- [x] 单测覆盖已关闭幂等。
- [x] 单测覆盖缺少 `yunxiao_task_id`。
- [x] 单测覆盖云效 API 失败。
- [x] 单测覆盖密钥不进入回写内容和 event payload。

联调验收：

- [x] 使用测试云效工作项完成一次回写和关闭。
- [x] 云效页面能看到 Adapter 回写内容。
- [x] Adapter workflow 查询能看到 `YUNXIAO_TASK_CLOSED`。

当前实现状态：

- 已在 `app/yunxiao.py` 实现 `GetWorkItemInfo -> CreateWorkitemComment -> UpdateWorkitemField/UpdateWorkItem -> GetWorkItemInfo`。
- 已在 `app/workflow.py` 接入 `APIFOX_SYNCED -> YUNXIAO_TASK_CLOSED`。
- 已在 `app/db.py` 增加 `YUNXIAO_TASK_CLOSED` 和 `NEEDS_HUMAN` 状态写入。
- 已扩展 `adapter_yunxiao_project_config` 和 `scripts/upsert_yunxiao_config.py` 维护关单字段。
- 已补 `POST /workflow/{workflow_id}/resolve`，用于把 `NEEDS_HUMAN` 恢复到 `APIFOX_SYNCED` 后重试关单。
- `done_status_id=100014` 已用于真实关单。
- 个人令牌链路已验证可完成详情查询、评论回写、状态更新和最终补偿校验。

2026-06-22 失败记录：

- `wf-d0e962010f1445ad` 已从 `CODING_REQUESTED -> CODE_SUBMITTED -> PIPELINE_SUCCESS -> APIFOX_SYNCED`。
- Apifox 导入返回 `201`，项目 ID 为 `8019331`。
- 调用 `/workflow/wf-d0e962010f1445ad/advance` 关单失败，workflow 进入 `NEEDS_HUMAN`。
- 失败原因：远端当前 `adapter_yunxiao_account_config` 只有 `legacy-openclaw`，`auth_type=legacy_token`，旧网关 `openapi-rdc.aliyuncs.com` 对工作项详情/评论/状态接口跳转到帮助文档，不是可用业务 API。
- 远端配置核对：`/etc/adapter-mvp/adapter-mvp.env` 未配置 `ALIBABA_CLOUD_*` / `ALIYUN_*` / `YUNXIAO_*`；`/root/.aliyun/config.json` 存在默认 profile，但 AK/Secret 为空，不能导入数据库复用。
- 解决方案：切换到云效个人令牌链路，使用 `openapi-rdc.aliyuncs.com` + `x-yunxiao-token` 并按项目配置 `spaceId`、`workitemTypeId`、`done_status_id`。
...
- Apifox 项目：`8460173`
- Apifox 导入结果：HTTP `201`
- 云效完成状态：`已完成 / 100014`
- workflow 最终状态：`YUNXIAO_TASK_CLOSED`
- 关单链路：查询工作项 -> 追加交付评论 -> 更新完成状态 -> 再次查询确认
