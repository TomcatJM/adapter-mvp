...
`adapter_api_client` 不保存明文 token；如需防误删恢复，配置
`ADAPTER_API_TOKEN_ENCRYPTION_KEY` 后使用 `scripts/upsert_api_client.py`
写入加密备份字段 `token_ciphertext`，并用 `token_last4` 做人工核对。
