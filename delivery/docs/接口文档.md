...
`adapter_api_client` 同时保存 `token_hash` 和 `token_plaintext`：
`token_hash` 用于鉴权比对，`token_plaintext` 用于误删后恢复。不要把
`token_plaintext` 打印到日志、提交到代码或粘贴到聊天。
...
## AI Phase 1 Planned APIs

These APIs belong to the AI研发闭环一期 plan. They are documented here as the target contract for parallel implementation; until the related code is merged, treat them as planned APIs rather than available production endpoints.
...
- `projectKey` must match `adapter_project_config`.
- Missing project config or missing `knowledge_endpoint` must fail explicitly.
- Adapter must not infer a default project.

### CodeGraph Index Callback
...
- Callback writes metadata to `adapter_codegraph_index`; CodeGraph index files stay in OSS, not Git or MySQL.
- Duplicate callbacks for the same `projectKey + branchName + commitId + indexVersion` must be idempotent.
- Failure callbacks should still be recorded with `indexStatus=failed` and an error message.
- If `workflowId` is present, the implementation may advance that workflow to `CODEGRAPH_INDEXED`; otherwise it only records the index version and must not guess a workflow.

### knowledgeContext Shape

Workflow context will store the normalized AI execution context under `knowledgeContext`:
...
- `DELETE /yunxiao/workitems` is an admin cleanup API, not part of the normal workflow.
- Authorization must use an `adapter_api_client` database token with `yunxiao:delete`; the environment `ADAPTER_API_TOKEN` fallback is intentionally not accepted.
- Request accepts `projectName + workitemIds` or `workflowId`; it never deletes by fuzzy title or assignee matching.
- `dryRun` defaults to `true`; set `dryRun=false` only after reviewing `deletePlan`.
- `workflowId` mode deletes recorded child tasks first; parent requirements are included only when `includeDemands=true`.
- Yunxiao account auth must be `personal_token`, because the delete path uses Yunxiao personal-token REST API.
...
If `assigneeName` / `assigneeId` is provided but does not match `adapter_yunxiao_member` and `adapter_yunxiao_project_member_relation`, Yunxiao task creation fails explicitly instead of silently assigning the wrong person.
